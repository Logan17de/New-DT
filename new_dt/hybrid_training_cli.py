...
from . import hybrid_comparison as base
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("Hybrid model estimates:")
    print(
        "  shared attention + unique FFN: "
        f"{parameter_counts['shared_attn_unique_ffn']:,} parameters, "
        f"~{training_bytes['shared_attn_unique_ffn'] / 1024**3:.2f} GiB "
        "FP32 parameter/optimizer state"
    )
    print(
        "  unique attention + shared FFN: "
        f"{parameter_counts['unique_attn_shared_ffn']:,} parameters, "
        f"~{training_bytes['unique_attn_shared_ffn'] / 1024**3:.2f} GiB "
        "FP32 parameter/optimizer state"
    )
    print(
        "Both use the same sparse token embedding, dense untied LM head, RoPE, "
        "RMSNorm, SwiGLU, batches, and schedule. No SPRC or split/merge."
    )
    ...
    run_config = {
        ...
        "comparison": {
            ...
            "shared_attn_unique_ffn": (
                "one shared Q/K/V/O set per layer and independent per-token "
                "SwiGLU Up/Gate/Down matrices"
            ),
            "unique_attn_shared_ffn": (
                "independent per-token Q/K/V/O matrices and one shared "
                "SwiGLU Up/Gate/Down set per layer"
            ),
        ...
        "fairness": {
            ...
            "same_embedding_and_untied_output_head_design": True,
            "gradient_clipping": (
                "sparse token-owned parameters and dense shared/output "
                "parameters are clipped independently using the same limit"
            ),
            "optimizer_note": (
                "SparseAdam updates active token-owned rows; AdamW updates the "
                "shared component, RMSNorm, and dense untied output head."
            ),
        },
    }
    ...
    names = base.model_names_from_args(args)
    print(
